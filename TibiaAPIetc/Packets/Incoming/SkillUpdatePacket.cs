...
namespace Tibia.Packets
{
    public class SkillUpdatePacket : Packet
    {
        ...
        public SkillUpdatePacket( byte[] data)
            : this()
        {
            ParseData(data);
        }
        ...
        public static SkillUpdatePacket Create( byte fist, byte fistper, byte club, byte clubper, byte sword, byte swordper, byte axe, byte axeper, byte distance, byte distanceper, byte shield, byte shieldper, byte fish, byte fishper)
        {
            PacketBuilder p = new PacketBuilder(PacketType.SkillUpdate);
            p.AddByte(fist);
            p.AddByte(fistper);
            p.AddByte(club);
            p.AddByte(clubper);
            ...
            p.AddByte(fish);
            p.AddByte(fishper);
            return new SkillUpdatePacket(p.GetPacket());
        }
    }
}